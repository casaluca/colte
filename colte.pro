...
;-
  ...
  if ntot ge 1 then begin
     ...
     if keyword_set(MC) then begin
       ...
       ; set to zero, then reasonable default values are assigned. 
        if keyword_set(elogg) then begin
           nbad   = 0
           elogg  = elogg(ok)
           bad    = where(elogg le 0. or finite(elogg) eq 0,nbad)
           if nbad ge 1 then elogg(bad) = 0.2
        endif else begin
           elogg=fltarr(ntot)+0.2
        endelse
     
        if keyword_set(efeh) then begin
           nbad  = 0
           efeh  = efeh(ok)
           bad   = where(efeh le 0. or finite(efeh) eq 0,nbad)
           if nbad ge 1 then efeh(bad) = 0.1
        endif else begin
           efeh=fltarr(ntot)+0.1
        endelse

        if keyword_set(egg) then begin
           nbad = 0        
           egg  = egg(ok)
           bad  = where(egg le 0. or finite(egg) eq 0,nbad)
           if nbad ge 1 then egg(bad) = 0.005
        endif else begin
           egg=fltarr(ntot)+0.005
        endelse
        
        if keyword_set(ebp) then begin
           nbad = 0        
           ebp  = ebp(ok)
           bad  = where(ebp le 0. or finite(ebp) eq 0,nbad)
           if nbad ge 1 then ebp(bad) = 0.005
        endif else begin
           ebp=fltarr(ntot)+0.005
        endelse
        
        if keyword_set(erp) then begin
           nbad = 0        
           erp  = erp(ok)
           bad  = where(erp le 0. or finite(erp) eq 0,nbad)
           if nbad ge 1 then erp(bad) = 0.005
        endif else begin
           erp=fltarr(ntot)+0.005
        endelse
        ...
        if keyword_set(eebv) then begin
           nbad = 0        
           eebv = eebv(ok)
           bad  = where(eebv le 0. or finite(eebv) eq 0,nbad)
           if nbad ge 1 then eebv(bad) = 0.1*ebv(bad)
        endif else begin
           eebv=0.1*ebv
        endelse
     ...
     if keyword_set(ej2) then begin
        nbad = 0        
        ej2  = ej2(ok)
        bad  = where(ej2 le 0. or finite(ej2) eq 0,nbad)
        if nbad ge 1 then ej2(bad) = 0.022
     endif else begin
        ej2=fltarr(ntot)+0.022
     endelse
     
     if keyword_set(eh2) then begin
        nbad = 0        
        eh2  = eh2(ok)
        bad  = where(eh2 le 0. or finite(eh2) eq 0,nbad)
        if nbad ge 1 then eh2(bad) = 0.024
     endif else begin
        eh2=fltarr(ntot)+0.024
     endelse
     
     if keyword_set(ek2) then begin
        nbad = 0        
        ek2  = ek2(ok)
        bad  = where(ek2 le 0. or finite(ek2) eq 0,nbad)
        if nbad ge 1 then ek2(bad) = 0.022
     endif else begin
        ek2=fltarr(ntot)+0.022
     endelse
     
     ; remove Gaia G<6 because of uncalibrated CCD saturation
     nog = where(gg lt 6.0,nnog)
     if nnog ge 1 then gg(nog) = !values.f_nan
     
     ; correct Gaia G as per Maiz Apellaniz & Weiler (2018)
     gp = gg - 0.0032*(gg-6.)
     
     ; more house-cleaning to remove 2MASS photometry likely to be bad
     noj = where(j2 lt 5.0 or ej2 gt 0.05,nnoj)
     noh = where(h2 lt 4.8 or eh2 gt 0.05,nnoh)
     nok = where(k2 lt 4.2 or ek2 gt 0.05,nnok)
     if nnoj ge 1 then j2(noj) = !values.f_nan
     if nnoh ge 1 then h2(noh) = !values.f_nan
     if nnok ge 1 then k2(nok) = !values.f_nan
     ...
     for i=long(0),ntot-1 do begin
        ...
        if keyword_set(MC) then begin
           ...
           dfeh    = feh [i]   + efeh[i] *randomn(seed,trials)
           dlog    = logg[i]   + elogg[i]*randomn(seed,trials)
           
           dcol[0,*]  = clr0[0,i]  + ebp[i]*randomn(seed,trials)-erp[i]*randomn(seed,trials) - (R_bp[i]-R_rp[i])*eebv[i]*randomn(seed,trials)
           dcol[1,*]  = clr0[1,i]  + ebp[i]*randomn(seed,trials)-ej2[i]*randomn(seed,trials) - (R_bp[i]-R_j2[i])*eebv[i]*randomn(seed,trials)
           dcol[2,*]  = clr0[2,i]  + ebp[i]*randomn(seed,trials)-eh2[i]*randomn(seed,trials) - (R_bp[i]-R_h2[i])*eebv[i]*randomn(seed,trials)
           dcol[3,*]  = clr0[3,i]  + ebp[i]*randomn(seed,trials)-ek2[i]*randomn(seed,trials) - (R_bp[i]-R_k2[i])*eebv[i]*randomn(seed,trials)
           
           dcol[4,*]  = clr0[4,i]  + erp[i]*randomn(seed,trials)-ej2[i]*randomn(seed,trials) - (R_rp[i]-R_j2[i])*eebv[i]*randomn(seed,trials)
           dcol[5,*]  = clr0[5,i]  + erp[i]*randomn(seed,trials)-eh2[i]*randomn(seed,trials) - (R_rp[i]-R_h2[i])*eebv[i]*randomn(seed,trials)
           dcol[6,*]  = clr0[6,i]  + erp[i]*randomn(seed,trials)-ek2[i]*randomn(seed,trials) - (R_rp[i]-R_k2[i])*eebv[i]*randomn(seed,trials)
           
           dcol[7,*]  = clr0[7,i]  + egg[i]*randomn(seed,trials)-ej2[i]*randomn(seed,trials) - (R_gg[i]-R_j2[i])*eebv[i]*randomn(seed,trials)
           dcol[8,*]  = clr0[8,i]  + egg[i]*randomn(seed,trials)-eh2[i]*randomn(seed,trials) - (R_gg[i]-R_h2[i])*eebv[i]*randomn(seed,trials)
           dcol[9,*]  = clr0[9,i]  + egg[i]*randomn(seed,trials)-ek2[i]*randomn(seed,trials) - (R_gg[i]-R_k2[i])*eebv[i]*randomn(seed,trials)
           
           dcol[10,*] = clr0[10,i] + egg[i]*randomn(seed,trials)-ebp[i]*randomn(seed,trials) - (R_gg[i]-R_bp[i])*eebv[i]*randomn(seed,trials)
           dcol[11,*] = clr0[11,i] + egg[i]*randomn(seed,trials)-erp[i]*randomn(seed,trials) - (R_gg[i]-R_rp[i])*eebv[i]*randomn(seed,trials)
        ...
        for j=0,11 do begin
           ...
           if (logg[i] gt 3.2 and (clr0[j,i] gt d_r[j] or clr0[j,i] lt d_b[j])) or $
              (finite(teff_cal[j,i]) eq 0) then begin
              teff_cal[j,i] = !values.f_nan
              dump = j
           endif
           
           if (logg[i] le 3.2 and (clr0[j,i] gt g_r[j] or clr0[j,i] lt g_b[j])) or $
              (finite(teff_cal[j,i]) eq 0) then begin
              teff_cal[j,i] = !values.f_nan
              dump = j              
           endif
        ...
        if keyword_set(MC) then begin
           ...
           if nav ge 1 then begin
              ...
              printf,lun,sid[i],logg[i],feh[i],ebv[i],$
                     ...
                     wteff,wse,$
                     format=outfor else $
              printf,lun,sid[i],logg[i],feh[i],ebv[i],wteff,wse,$
                     format=outfor
           ...
           if nav ge 1 then begin
              ...
              printf,lun,sid[i],logg[i],feh[i],ebv[i],teff_cal[0,i],teff_cal[1,i],teff_cal[2,i],teff_cal[3,i],teff_cal[4,i],teff_cal[5,i],teff_cal[6,i],teff_cal[7,i],teff_cal[8,i],teff_cal[9,i],teff_cal[10,i],teff_cal[11,i],format=outfor 
           endif
